...
def substrings(string)
  subs = []
  word_ary = string.split("")
  word_ary.each_index do |n|
    word_ary.each_index do |x|
      j = x + 1
      sub = word_ary.slice(n, j).join("")
...
class Array
  def my_map(&prc)
    mod_array = []
    self.my_each { |n| mod_array << prc.call(n) }
    mod_array
  end

  def my_select(&prc)
    select_array = []
    self.my_each { |n| select_array << n if prc.call(n) }
    select_array
  end

  def my_inject(&blk)
    sum ||= self.first
    self.drop(1).my_each { |num| sum = blk.call(sum, num) }
    sum
  end
end
...
def concatenate(strings)
  string = ""
  strings.inject { |memo, word| memo + word }
end
